...
class batch_temp(object):
    ...
    CHECK_INTERVAL_VALUES = {}  # Store custom check interval values (in seconds) per user
...
async def auto_monitor_channels(client: Client, user_id: int):
    ...
    await client.send_message(
        ...
        f"⏰ Check intervals: Random selection from {len(check_values)} values\n"
...
async def set_check_interval(client: Client, message: Message):
    try:
        ...
        if not parts:
            await message.reply(
                "**Usage:** `/setcheckinterval <seconds1> <seconds2> <seconds3> ...`\n\n"
        ...
        for part in parts:
            try:
                ...
                if 0 <= val <= 86400:
                    ...
                    await message.reply(f"❌ Invalid value: {val}. Must be between 0-86400 seconds.")
                    return
            except ValueError:
                await message.reply(f"❌ Invalid value: {part}. Must be a number.")
                return
        
        if not interval_values:
            await message.reply("❌ No valid interval values provided!")
            return
...
async def get_check_interval(client: Client, message: Message):
    intervals = batch_temp.CHECK_INTERVAL_VALUES.get(
