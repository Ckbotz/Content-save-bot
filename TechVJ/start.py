...
class batch_temp(object):
    ...
    FILTER_WORDS = {}  # Store filename filter words per user
    PERMANENT_THUMBNAIL = {}  # Store permanent thumbnail URL per user
...
def filter_filename(filename, user_id):
    """Remove filter words from filename"""
    if not filename:
        return filename
        
    filter_words = batch_temp.FILTER_WORDS.get(user_id, [])
    if not filter_words:
        return filename
    
    original_filename = filename
    for word in filter_words:
        filename = filename.replace(word, "").replace("  ", " ").strip()
    
    # If filename becomes empty after filtering, return original
...
async def download_permanent_thumbnail(thumbnail_url, user_id):
    """Download permanent thumbnail from URL"""
    try:
        import requests
        ...
        thumbnail_path = f"thumbnails/{user_id}{file_ext}"
...
async def send_help(client: Client, message: Message):
    help_text = f"{HELP_TXT}\n\n**🕐 Custom Sleep Settings:**\n" \
                ...
                f"**📝 Filename Filter:**\n" \
                f"Use `/filterwords` to set words to remove from filenames\n\n" \
                f"**🖼️ Permanent Thumbnail:**\n" \
                f"Use `/setthumb` to set a permanent thumbnail URL"
...
# Set filter words command
@Client.on_message(filters.command(["filterwords"]))
async def set_filter_words(client: Client, message: Message):
    try:
        parts = message.text.split()[1:]
        if not parts:
            # Show current filter words
            current_words = batch_temp.FILTER_WORDS.get(message.from_user.id, [])
            if current_words:
                await message.reply(
                    f"**Current Filter Words:**\n`{', '.join(current_words)}`\n\n"
                    f"To set new words: `/filterwords word1 word2 word3`\n"
                    f"To clear: `/filterwords clear`"
                )
            else:
                await message.reply(
                    "**No filter words set.**\n\n"
                    "**Usage:** `/filterwords word1 word2 word3`\n"
                    "These words will be removed from downloaded filenames.\n\n"
                    "**Example:** `/filterwords sample spam ads`\n"
                    "To clear: `/filterwords clear`"
                )
            return
        
        if parts[0].lower() == "clear":
            batch_temp.FILTER_WORDS[message.from_user.id] = []
            await message.reply("✅ **Filter words cleared successfully!**")
            return
        
        filter_words = [word.strip() for word in parts if word.strip()]
        batch_temp.FILTER_WORDS[message.from_user.id] = filter_words
        
        await message.reply(
            f"✅ **Filter words set successfully!**\n\n"
            f"Words: `{', '.join(filter_words)}`\n\n"
            f"These words will be automatically removed from downloaded filenames.\n"
            f"Use `/filterwords clear` to remove all filter words."
        )
    except Exception as e:
        await message.reply(f"❌ Error setting filter words: {e}")


# Set permanent thumbnail command
...
async def set_thumbnail(client: Client, message: Message):
    try:
        ...
        if not parts:
            ...
            current_thumb = batch_temp.PERMANENT_THUMBNAIL.get(message.from_user.id)
            if current_thumb:
                await message.reply(
                    f"**✅ Permanent thumbnail is set.**\n\n"
                    f"URL: `{current_thumb}`\n\n"
                    f"To remove: `/setthumb remove`\n"
                    f"To change: `/setthumb new_url_here`"
                )
            else:
                await message.reply(
                    "**No permanent thumbnail set.**\n\n"
                    "**Usage:** `/setthumb thumbnail_url`\n"
                    "Set a permanent thumbnail for all uploaded media.\n\n"
                    "**Example:** `/setthumb https://example.com/thumb.jpg`\n"
                    "To remove: `/setthumb remove`"
                )
            return
        
        if parts[0].lower() == "remove":
            batch_temp.PERMANENT_THUMBNAIL[message.from_user.id] = None
            # Remove downloaded thumbnail file if exists
            thumb_path = f"thumbnails/{message.from_user.id}.jpg"
            if os.path.exists(thumb_path):
                os.remove(thumb_path)
            await message.reply("✅ **Permanent thumbnail removed successfully!**")
            return
        
        thumbnail_url = " ".join(parts)
        
        # Validate URL
        if not thumbnail_url.startswith(('http://', 'https://')):
            await message.reply("❌ Please provide a valid HTTP/HTTPS URL!")
            return
        
        # Download and verify thumbnail
        downloaded_path = await download_permanent_thumbnail(thumbnail_url, message.from_user.id)
        if downloaded_path:
            batch_temp.PERMANENT_THUMBNAIL[message.from_user.id] = thumbnail_url
            await message.reply(
                f"✅ **Permanent thumbnail set successfully!**\n\n"
                f"URL: `{thumbnail_url}`\n\n"
                f"This thumbnail will be used for all uploaded media.\n"
                f"Use `/setthumb remove` to remove it."
            )
        else:
            await message.reply("❌ Failed to download thumbnail. Please check the URL and try again.")
            
    except Exception as e:
        await message.reply(f"❌ Error setting thumbnail: {e}")
...
async def handle_private(client: Client, acc, message: Message, chatid: int, msgid: int):
    ...
    # Apply filename filtering for documents and videos
    if msg_type in ["Document", "Video"]:
        original_filename = os.path.basename(file)
        filtered_filename = filter_filename(original_filename, message.from_user.id)
        
        if original_filename != filtered_filename:
            # Rename the file with filtered filename
            new_file_path = os.path.join(os.path.dirname(file), filtered_filename)
            os.rename(file, new_file_path)
            file = new_file_path

    # Get permanent thumbnail if set
    permanent_thumb = None
    thumbnail_url = batch_temp.PERMANENT_THUMBNAIL.get(message.from_user.id)
    if thumbnail_url:
        thumb_path = f"thumbnails/{message.from_user.id}.jpg"
        if os.path.exists(thumb_path):
            permanent_thumb = thumb_path

    if msg_type == "Document":
        try:
            # Use permanent thumbnail if available, otherwise get from document
            if permanent_thumb:
...
# 👇 ensure this elif is aligned with the previous if (not inside the try)
    ...
    try:
        ...
        try:
            # Use permanent thumbnail if available, otherwise get from audio
            if permanent_thumb:
                ph_path = permanent_thumb
            else:
                try:
