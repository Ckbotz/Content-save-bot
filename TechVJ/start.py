...
LOG_CHANNEL_ID = -1001989242533  # Replace with your log channel ID (must be negative for channels/groups)
...
def get_filename_without_extension(filename):
    ...
    # Split and return name without extension
    name_parts = filename.rsplit('.', 1)
    return name_parts[0]


def create_custom_caption(filename, original_caption=None):
    """Create custom caption with cleaned filename"""
    # Get cleaned filename without extension
    clean_name = get_filename_without_extension(filename)
...
async def process_document(client: Client, acc, user_id: int, msg):
    ...
    # Create custom caption with cleaned filename
    custom_caption = create_custom_caption(os.path.basename(file) if file else "File")
    sent_message = None
    ...
    try:
        ...
        # Upload document with custom caption
